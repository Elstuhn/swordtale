...
class Mob():
    def __init__(self, name, level, atk, hp, defense, phys_atk, phys_def, mag_atk, mag_def, cooldown_speed): #atk, defense, hp will be calculated as constant*multiplier
        self.name = name
        self.level = level
        self.atk = atk
        self.hp = hp
...
async def monsterattack(ctx, playerins, dmg, mob):
...
async def taskbattle(seconds, func, ctx, playerins, dmg, mob):
        while True:
            _ = await func(ctx, playerins, dmg, mob)
            if playerins.stats['hp'] < 1:
                return 0
            await asyncio.sleep(seconds)
    

async def returntaskmobattack(seconds, func, ctx, playerins, dmg, mob):
...
async def player(ctx):
    ...
    _ = []
    embed.add_field(name = "Skill Level :low_brightness:", value = f"{t}")
    await ctx.send(embed= embed)
